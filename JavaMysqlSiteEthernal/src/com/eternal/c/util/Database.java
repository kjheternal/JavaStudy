...
public class Database {
	private static String DB_NAME = "my_siteboard";
...
//				String replyOri = Database.result.getString("b_reply_ori");
	...
	public static boolean isProcLogin(String id, String pw) {
		String count = "";
		try {
			String sql = String.format("select count(*) from member where s_id='%s' and s_pw='%s';", id, pw);
			
			Database.result = Database.st.executeQuery(sql); // SQL SELECT 구문 사용 시 쓰는 함수 result 객체에 담을 수 있다.
			Database.result.next();
			
			count = Database.result.getString("count(*)");
			
			Common.wn("찾은 아이디 수: " +count);
		} catch (SQLException e) {
			Common.wn("SQLException: " + e.getMessage());
			Common.wn("SQLState: " + e.getSQLState());
